...
import { authLimiter, adminLimiter, uploadLimiter } from '../middleware/rateLimiter';
...
describe('rate limiter middlewares', () => {
  it('bloqueia autenticação após 10 tentativas na janela', async () => {
    const app = express();
    app.post('/auth', authLimiter, (_req, res) => res.status(200).json({ ok: true }));
  ...
  it('bloqueia uploads após 30 tentativas na janela', async () => {
    const app = express();
    app.post('/upload', uploadLimiter, (_req, res) => res.status(200).json({ ok: true }));
  ...
  it('bloqueia endpoints admin após 60 tentativas na janela', async () => {
    const app = express();
    app.post('/admin', adminLimiter, (_req, res) => res.status(200).json({ ok: true }));
